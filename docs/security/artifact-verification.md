...
macOS package は `codesign --verify --deep --strict` と `spctl --assess --type open` で署名・
公証を確認する。Sparkle / WinSparkleはOS側へ状態を持つため削除済みで、全OSを手動更新とする。
Windows packageはコード署名が未導入なのでmanifest上も`disabled-unsigned`とする。Linux AppImageはmanifest
のSHA-256を照合する。
