...
7. 公開後のappcast metadata jobがmanifestのdigest / size / version、EdDSA signature、downgradeを検証したことを確認する。macOSはSparkleで`appcast.xml`、WindowsはWinSparkleで`appcast-windows.xml`を読み、更新を通知する。
...
appcast署名鍵を失効した場合はmacOS appcastを再署名する。WindowsではWinSparkleのfeedである
`appcast-windows.xml`も撤回し、修正版releaseを公開する。Sparkle / WinSparkleの更新確認用
OS preference / registry値はAwayukiのユーザーデータではなく、資格情報・設定・Timelineは
`awayuki.db`だけに保存する。
